...
class RemoveCompositionFromCatalog extends Operation {
  ...
  async execute(user, catalogId, compositionId) {
    ...
    try {
      ...
      const catalog = await this.catalogRepository.removeComposition(catalogId, composition);
...
RemoveCompositionFromCatalog.setOutputs(['SUCCESS', 'ERROR', 'NOT_FOUND', 'FORBIDDEN']);

module.exports = RemoveCompositionFromCatalog;
